...
def main(prog_args: dict = None) -> None:
    """
    The primary entrypoint for validating a file or directory against the CASE schema by utilizing the pySHACL command
    line utility. Full functionality of this integration can be found in the module-level documentation.

    :param prog_args: a dictionary of key/value pairs that match the long names of the arguments defined by argparse.
        if the
    :return: None
    """
    ...
    # Override the arguments based on any provided by the passed in dictionary in prog_args (programmatic args)
    if prog_args is not None and isinstance(prog_args, dict):
        if "in_graph" in prog_args:
            args.in_graph = prog_args["in_graph"]
